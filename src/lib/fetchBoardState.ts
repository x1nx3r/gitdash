...
/**
 * Full board snapshot for a repo scope: GitHub search (open + merged today),
 * first-level enrichment (reviewers + diff stats), column derivation and the
 * mock fallback. Shared by the /api/github/prs route and the webhook-driven
 * cache refresher so both paths can never drift.
 *
 * GitHub API responses bypass Next's data cache (no-store): freshness is
 * governed by boardCache and the enrichment cache instead, and a webhook
 * rebuild must never read a stale HTTP-cached search result.
 */
export async function fetchBoardState(includedRepos: string[]): Promise<GitHubApiResponse> {
